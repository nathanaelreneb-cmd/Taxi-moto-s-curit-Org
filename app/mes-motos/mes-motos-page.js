...
const labels = {
  ...
  signale_chauffeur: 'Signalée — en attente de confirmation par un agent',
...
export default function MesMotos() {
  ...
  const [ficheMoto, setFicheMoto] = useState(null);
  ...
  async function handleAjouter(e) {
    e.preventDefault();
    setAddError('');
    const { data, error } = await supabase.rpc('ajouter_moto_personnelle', {
    ...
    if (error) {
      setAddError("Une erreur est survenue, réessayez.");
      return;
    }
    if (!data) {
      setAddError(
        "Impossible de vérifier votre identité — reconnectez-vous depuis le début avec le nom, le téléphone et la CNI exacts."
      );
      return;
    }
    ...
    <div className="shell" style={{ maxWidth: 460 }}>
            ...
            <div className="card" key={m.id}>
              ...
              <div className="row" style={{ marginTop: 6 }}>
                <button className="btn-ghost" onClick={() => handlePrintFiche(m)}>
                  Imprimer / télécharger ma fiche
                </button>
              </div>
              {m.type_moto === 'personnel' && (
                <div className="row" style={{ marginTop: 10 }}>
                  {(m.statut === 'volee' || m.statut === 'signale_chauffeur') ? (
              ...
              {m.type_moto === 'personnel' && m.statut === 'signale_chauffeur' && (
                <p className="hint">
                  Votre déclaration est enregistrée. Pour des raisons de sécurité, le suivi GPS
                  ne s&rsquo;active qu&rsquo;après confirmation par un agent — rendez-vous à une
                  gare si possible pour accélérer la vérification.
                </p>
              )}
              {m.type_moto === 'taxi' && (
                <p className="hint">
          ...
          <div className="card">
            {!showAdd ? (
              ...
              <form onSubmit={handleAjouter}>
                ...
                {addError && <p className="hint" style={{ color: 'var(--rejected)' }}>{addError}</p>}
      ...
      {ficheMoto && identite && (
        <div className="print-only">
          ...
          <p>À conserver par le chauffeur. En cas de perte ou de vol, présentez cette fiche
          à votre gare ou scannez le code ci-dessous.</p>
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 16 }}>
            <tbody>
              <tr><td style={{ padding: 6, fontWeight: 'bold' }}>Nom</td><td style={{ padding: 6 }}>{identite.nom}</td></tr>
              <tr><td style={{ padding: 6, fontWeight: 'bold' }}>Téléphone</td><td style={{ padding: 6 }}>{identite.telephone}</td></tr>
              <tr><td style={{ padding: 6, fontWeight: 'bold' }}>CNI</td><td style={{ padding: 6 }}>{identite.cni}</td></tr>
              <tr><td style={{ padding: 6, fontWeight: 'bold' }}>Plaque</td><td style={{ padding: 6 }}>{ficheMoto.plaque_immatriculation}</td></tr>
              <tr><td style={{ padding: 6, fontWeight: 'bold' }}>Type</td><td style={{ padding: 6 }}>{ficheMoto.type_moto === 'personnel' ? 'Moto personnelle' : 'Moto de taxi'}</td></tr>
            </tbody>
          </table>
          <div style={{ marginTop: 20 }}>
            <QRCodeSVG
              value={`${typeof window !== 'undefined' ? window.location.origin : ''}/signaler?nom=${encodeURIComponent(
                identite.nom
              )}&telephone=${encodeURIComponent(identite.telephone)}&cni=${encodeURIComponent(
                identite.cni
              )}&plaque=${encodeURIComponent(ficheMoto.plaque_immatriculation)}`}
              size={140}
            />
