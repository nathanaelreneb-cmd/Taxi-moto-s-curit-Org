    ...
    <div className="shell">
            ...
            <div className="entry" key={m.id}>
              ...
              {open && (
                <>
                  ...
                  <div className="row" style={{ marginTop: 10 }}>
                    <QRCodeSVG
                      value={`${typeof window !== 'undefined' ? window.location.origin : ''}/verifier?code=${m.qr_code}`}
                      size={44}
                      bgColor="transparent"
                      fgColor="#f2ede1"
                    />
      ...
      {ficheAImprimer && (
        <div className="print-only">
          ...
          <div style={{ marginTop: 24, borderTop: '1px dashed #999', paddingTop: 16 }}>
            <p style={{ fontWeight: 'bold' }}>
              ✂️ À découper et coller discrètement sur la moto (sous le siège, dans le carénage...)
            </p>
            <QRCodeSVG
              value={`${typeof window !== 'undefined' ? window.location.origin : ''}/verifier?code=${ficheAImprimer.qr_code}`}
              size={100}
            />
            <p style={{ fontSize: 11 }}>
              Ce code permet à toute personne de vérifier si la moto est en règle, sans jamais
              révéler vos informations personnelles. Collez-en plusieurs, à des endroits différents.
            </p>
          </div>
        </div>
      )}
